...
void EXPORT_CALL handler_ShowVsNames(u32 id, void* win, void* puser)
{
	if (settings.OSD.ShowVsNames)
		settings.OSD.ShowVsNames = 0;
	else
		settings.OSD.ShowVsNames = 1;

	emu.SetMenuItemStyle(id, settings.OSD.ShowVsNames ? MIS_Checked : 0, MIS_Checked);
...
s32 FASTCALL Load(emu_info* emu_inf)
{
	...
	emu.AddMenuItem(emu.RootMenu,-1,L"Show Vs Names", handler_ShowVsNames, settings.OSD.ShowVsNames);
...
void LoadSettings()
{
	...
	settings.OSD.ShowVsNames					=	cfgGetInt(L"OSD.ShowVsNames", 1);

	settings.Enhancements.ShowGameName			=	cfgGetInt(L"ShowGameName", 0);
