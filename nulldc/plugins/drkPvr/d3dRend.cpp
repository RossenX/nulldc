...
__error_out:
	...
	void DrawOSD_Text(RECT _rect, LPCWSTR _text, DWORD _format) {
		...
		plate_font->DrawText(NULL, _text, -1, &rct_offset1, _format, D3DCOLOR_ARGB(0xFF, 0x00, 0x00, 0x00));
		plate_font->DrawText(NULL, _text, -1, &_rect, _format, D3DCOLOR_ARGB(0xFF, 0xD0, 0xD0, 0xD0));
	...
	void DrawOSD()
	{
		...
		if (settings.OSD.ShowVsNames) {
			...
			wchar P1andP2name[512];
			wsprintf(P1andP2name, L"%s vs %s", p1Name, p2Name);
			// Left Top Right Down
			RECT vsOffset;
			vsOffset.top = 0;
			vsOffset.left = 0;
			vsOffset.right = rect.right - rect.left;
			vsOffset.bottom = 30;

			DrawOSD_Text(vsOffset, L"vs", DT_CENTER);

			RECT p1Offset;
			p1Offset.right = ((rect.right - rect.left) / 2) - 15;
			p1Offset.bottom = 30;
			DrawOSD_Text(p1Offset, p1Name,DT_RIGHT);

			RECT p2Offset;
			p2Offset.left = ((rect.right - rect.left) / 2) + 15;
			p2Offset.bottom = 30;
			DrawOSD_Text(p2Offset, p2Name,DT_LEFT);
...
//			printf("ZMAE: %f %f\n",c0[0],c1[0]);
	...
	u32 THREADCALL RenderThead_internal(void* param)
	{
		...
		D3DXCreateFont(dev, 18, 0, FW_BLACK, 0, FALSE, DEFAULT_CHARSET,
