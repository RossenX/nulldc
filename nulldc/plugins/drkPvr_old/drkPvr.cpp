...
void handler_VsNames(int mode)
{

	settings.OSD.ShowVsNames = mode;
	SaveSettings();
}
...
s32 FASTCALL Load(emu_info* emu_inf)
{
	...
	menu_resolution.Add(RSM,L"Up to 1920x1080 (Most Stable)",0);
	menu_resolution.Add(RSM,L"Up to 1280x800",1);
	...
	menu_resolution.Add(RSM,L"Maximum Possible (Highest Quality)", 5);
	...
	u32 VNM = emu.AddMenuItem(emu.RootMenu, -1, L"Vs Names: %s", 0, 0);
	menu_VsNames.format = L"Vs Names: %s";
	menu_VsNames.callback = handler_VsNames;

	menu_VsNames.Add(VNM, L"None", 0);
	menu_VsNames.Add(VNM, L"Top Full", 1);
	menu_VsNames.Add(VNM, L"Top Minimal", 2);
	menu_VsNames.Add(VNM, L"Bottom Full", 3);
	menu_VsNames.Add(VNM, L"Bottom Minimal", 4);
	menu_VsNames.SetValue(settings.OSD.ShowVsNames);
...
void LoadSettings()
{
	...
	settings.OSD.ShowVsNames					=	cfgGetInt(L"OSD.ShowVsNames", 3);
	

	emu.ConfigLoadStr(L"BEARPlay", L"P1Name", p1Name, L"P1");
	emu.ConfigLoadStr(L"BEARPlay", L"P2Name", p2Name, L"P2");
	emu.ConfigLoadStr(L"BEARPlay", L"GameName", GameName, L"None");
	//for (int i = 4; i < 512; ++i){GameName[i - 4] = GameName[i];}
	...
	settings.Enhancements.ShowGameName			=	cfgGetInt(L"Enhancements.ShowGameName", 1);
}
...
void SaveSettings()
{
	...
	cfgSetInt(L"OSD.ShowVsNames", settings.OSD.ShowVsNames);
