...
#define PS_SHADER_COUNT (384*4)
	...
	ID3DXFont* font;
	ID3DXFont* plate_font;
	ID3DXFont* plate_font2;
	LPDIRECT3DTEXTURE9 tex_fullScoreBoardBackground;
	D3DXIMAGE_INFO tex_fullScoreBoardBackgroundInfo;
	LPD3DXSPRITE spr_FullScoreBoard;
	LPDIRECT3DTEXTURE9 tex_fullScoreBoardBackground2;
	D3DXIMAGE_INFO tex_fullScoreBoardBackgroundInfo2;
	...
	void HandleEvent(u32 evid,void* p)
	{
		if (evid == NDE_GUI_RESIZED)
		{
			...
			if (p) memcpy((void*)&resizerq.new_size,p,sizeof(NDC_WINDOW_RECT));
			resizerq.rev++;
		}
		/**/
		if (evid == NDE_PVR_RECORD) {
			memcpy((void*)&RecordInput, p, sizeof(bool[2]));
		}

		if (evid == NDE_PVR_PLAYBACK) {
			memcpy((void*)&PlaybackInput, p, sizeof(bool[2]));
		}
		/**/
		// Key Display Here
...
__error_out:
	...
	void DrawOSD_Text(RECT _rect, LPCWSTR _text, DWORD _format, int _color) {

		RECT rct_offset1;
		memcpy(&rct_offset1, &_rect, sizeof(_rect));

		rct_offset1.left += 2;
		rct_offset1.top += 2;
		rct_offset1.right += 2;
		rct_offset1.bottom += 2;
		int txtColor[3];
		...
		plate_font->DrawText(NULL, _text, -1, &rct_offset1, _format, D3DCOLOR_ARGB(255, 0, 0, 0));
		plate_font->DrawText(NULL, _text, -1, &_rect, _format, D3DCOLOR_ARGB(255, txtColor[0], txtColor[1], txtColor[2]));

	}

	void DrawOSD_Text2(RECT _rect, LPCWSTR _text, DWORD _format, int _color) {
		...
		plate_font2->DrawText(NULL, _text, -1, &rct_offset1, _format, D3DCOLOR_ARGB(255, 0, 0, 0));
		plate_font2->DrawText(NULL, _text, -1, &_rect, _format, D3DCOLOR_ARGB(255, txtColor[0], txtColor[1], txtColor[2]));
	...
	void DrawOSD()
	{
		...
		if (RecordInput[0])
		{
			RECT Backdrop_Rect = { 0, 0, 500, 500 };
			DrawOSD_Text(Backdrop_Rect, L"Record P2", DT_LEFT, 0);
		...
		{
			RECT Backdrop_Rect = { 0, 0, 500, 500 };
			DrawOSD_Text(Backdrop_Rect, L"Record P1", DT_LEFT, 0);
		...
		{
			RECT Backdrop_Rect = { 0, 0, 500, 500 };
			DrawOSD_Text(Backdrop_Rect, L"Playback P2", DT_LEFT, 0);
		...
		{
			RECT Backdrop_Rect = { 0, 0, 500, 500 };
			DrawOSD_Text(Backdrop_Rect, L"Playback P1", DT_LEFT, 0);

		}
		/**/

		// PLAYER NAMEPLAYED
		if (settings.OSD.ShowVsNames) {
			...
			RECT Backdrop_Rect = { 0, 0, 0, 0};
			D3DXVECTOR3 pCenter = D3DXVECTOR3(512, 0, 0);
			D3DXVECTOR3 BackgroundVector = D3DXVECTOR3(0, 0, 0);

			D3DXMATRIX VsNameScale;
			D3DXMatrixScaling(&VsNameScale, VsScale, VsScale, VsScale);

			if (settings.OSD.ShowVsNames == 1) {

				BackgroundVector.x = (vp.Width / 2) * (1 / VsScale);

				Backdrop_Rect.right = tex_fullScoreBoardBackgroundInfo.Width;
				Backdrop_Rect.bottom = tex_fullScoreBoardBackgroundInfo.Height;

				pCenter.x = tex_fullScoreBoardBackgroundInfo.Width /2;

				spr_FullScoreBoard->Begin(D3DXSPRITE_DONOTSAVESTATE);
				spr_FullScoreBoard->SetTransform(&VsNameScale);
				spr_FullScoreBoard->Draw(tex_fullScoreBoardBackground, &Backdrop_Rect, &pCenter, &BackgroundVector, 0xFFFFFFFF);
				spr_FullScoreBoard->End();

				p1Offset.right = (vp.Width / 2) - (45 * VsScale);
				p1Offset.left = (vp.Width / 2) - (230 * VsScale);
				p1Offset.bottom = (20 * VsScale);
				p1Offset.top = -1;
				DrawOSD_Text(p1Offset, p1Name, DT_CENTER, 0);

				p2Offset.left = (vp.Width / 2) + (45 * VsScale);
				p2Offset.right = (vp.Width / 2) + (230 * VsScale);
				p2Offset.bottom = (20 * VsScale);
				p2Offset.top = -1;
				DrawOSD_Text(p2Offset, p2Name, DT_CENTER, 0);
			}
			else if (settings.OSD.ShowVsNames == 2) {
				vsOffset.top = 0;
				vsOffset.left = 0;
				vsOffset.right = vp.Width;
				vsOffset.bottom = ((30 * VsScale) * 2.25);
				vsOffset.top = 1;
				DrawOSD_Text2(vsOffset, L"vs", DT_CENTER, 2);

				p1Offset.right = (vp.Width / 2) - ((15 * VsScale) * 2.25);
				p1Offset.left = 0;
				p1Offset.bottom = ((20 * VsScale) * 2.25);
				p1Offset.top = -1;
				DrawOSD_Text2(p1Offset, p1Name, DT_RIGHT, 0);

				p2Offset.left = (vp.Width / 2) + ((15 * VsScale) * 2.25);
				p2Offset.right = vp.Width;
				p2Offset.bottom = ((20 * VsScale) * 2.25);
				p2Offset.top = -1;
				DrawOSD_Text2(p2Offset, p2Name, DT_LEFT, 0);
			}
			else if (settings.OSD.ShowVsNames == 3) {

				BackgroundVector.x = (vp.Width / 2) * (1 / VsScale);
				BackgroundVector.y = vp.Height * (1 / VsScale);

				Backdrop_Rect.right = tex_fullScoreBoardBackgroundInfo2.Width;
				Backdrop_Rect.bottom = tex_fullScoreBoardBackgroundInfo2.Height;

				pCenter.x = tex_fullScoreBoardBackgroundInfo2.Width / 2;
				pCenter.y = tex_fullScoreBoardBackgroundInfo2.Height;

				spr_FullScoreBoard->Begin(D3DXSPRITE_DONOTSAVESTATE);
				spr_FullScoreBoard->SetTransform(&VsNameScale);
				spr_FullScoreBoard->Draw(tex_fullScoreBoardBackground2, &Backdrop_Rect, &pCenter, &BackgroundVector, 0xFFFFFFFF);
				spr_FullScoreBoard->End();


				p1Offset.right = (vp.Width / 2) - (45 * VsScale);
				p1Offset.left = (vp.Width / 2) - (230 * VsScale);
				p1Offset.top = vp.Height - (20 * VsScale);
				p1Offset.bottom = vp.Height;

				DrawOSD_Text(p1Offset, p1Name, DT_CENTER, 0);

				p2Offset.left = (vp.Width / 2) + (45 * VsScale);
				p2Offset.right = (vp.Width / 2) + (230 * VsScale);
				p2Offset.top = vp.Height - (20 * VsScale);
				p2Offset.bottom = vp.Height;

				DrawOSD_Text(p2Offset, p2Name, DT_CENTER, 0);

			}
			else if (settings.OSD.ShowVsNames == 4) {

				vsOffset.top = 0;
				vsOffset.left = 0;
				vsOffset.right = vp.Width;
				vsOffset.bottom = vp.Height;
				vsOffset.top = vp.Height - ((18 * VsScale) * 2.25);
				DrawOSD_Text2(vsOffset, L"vs", DT_CENTER, 2);

				p1Offset.right = (vp.Width / 2) - ((15 * VsScale) * 2.25);
				p1Offset.left = 0;
				p1Offset.top = vp.Height - ((18 * VsScale) * 2.25);
				p1Offset.bottom = vp.Height;
				DrawOSD_Text2(p1Offset, p1Name, DT_RIGHT, 0);

				p2Offset.left = (vp.Width / 2) + ((15 * VsScale) * 2.25);
				p2Offset.right = vp.Width;
				p2Offset.top = vp.Height - ((18 * VsScale) * 2.25);
				p2Offset.bottom = vp.Height;
				DrawOSD_Text2(p2Offset, p2Name, DT_LEFT, 0);
...
//			printf("ZMAE: %f %f\n",c0[0],c1[0]);
	...
	void Medidate_fb()
	{
		...
		case 0:
			if (win_height*win_width>2073600)
			{
				float rz = sqrtf(2073600.0f / (win_height*win_width));
	...
	u32 THREADCALL RenderThead_internal(void* param)
	{
		...
		dev->GetViewport(&vp);
		VsScale = (float)((float)vp.Height / 1080.0f);
		...
		D3DXCreateFont(dev, (20 * VsScale), 0, FW_BLACK, 0, FALSE, DEFAULT_CHARSET,
			OUT_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, TEXT("Arial"), &plate_font);

		D3DXCreateFont(dev, (20 * VsScale) * 2.25, 0, FW_BLACK, 0, FALSE, DEFAULT_CHARSET,
			OUT_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, TEXT("Arial"), &plate_font2);

		D3DXGetImageInfoFromFileA("Vs.png", &tex_fullScoreBoardBackgroundInfo);
		D3DXGetImageInfoFromFileA("Vs_2.png", &tex_fullScoreBoardBackgroundInfo2);

		D3DXCreateTextureFromFile(dev, L"Vs.png", &tex_fullScoreBoardBackground);
		D3DXCreateTextureFromFile(dev, L"Vs_2.png", &tex_fullScoreBoardBackground2);

		D3DXCreateSprite(dev, &spr_FullScoreBoard);
