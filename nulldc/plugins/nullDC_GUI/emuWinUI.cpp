...
void CreateBasicMenus()
{
	...
	gui_fullscreen_mid=AddMenuItem(menu_setts,-1,L"Fullscreen (Alt + Enter)",Handle_Options_Fullscreen,settings.Fullscreen);
	AddMenuItem(menu_setts,-1,L"Fullscreen on Top",Handle_Options_AlwaysOnTop,settings.AlwaysOnTop); 
...
void ToggleFullscreen(HWND hWnd)
{
	...
	if (GetWindowLong(hWnd,GWL_STYLE)&WS_CAPTION)
	{
		...
		if (settings.AlwaysOnTop == 1) {
			...
			SetWindowPos(hWnd, HWND_NOTOPMOST, mi.rcMonitor.left, mi.rcMonitor.top, mi.rcMonitor.right - mi.rcMonitor.left, mi.rcMonitor.bottom - mi.rcMonitor.top, 0);
		}
		
		//SetWindowPos(hWnd,NULL,mi.rcMonitor.left,mi.rcMonitor.top,mi.rcMonitor.right-mi.rcMonitor.left,mi.rcMonitor.bottom-mi.rcMonitor.top,0);
		SetMenu(hWnd, NULL);
	}
	else
	{
		...
		RECT r=oldrect;
		AdjustWindowRectEx(&r,GetWindowLong(hWnd,GWL_STYLE),GetMenu(hWnd)!=NULL,GetWindowLong(hWnd,GWL_EXSTYLE));
		SetWindowPos( hWnd, HWND_NOTOPMOST, oldrect.left, oldrect.top, r.right - r.left, r.bottom - r.top,0) ;
